...
test("Last Name requirements", async () => {
    render(<ContactForm />);
    ...
    const lNameError = await screen.findByText(/Looks like there was an error: required/i);
    expect(lNameError).toBeInTheDocument();
});

test("Email Name requirements", async () => {
    render(<ContactForm />);
    ...
    const emailError = await screen.findByText(/Looks like there was an error: required/i);
